...
## 方向性の大転換（2026-07-17）
- 最終ゴールを「予約システム」に変更（Airリザーブ参考）
- LINEやHPに専用URLを埋め込み → カレンダーから予約できる仕組み
- 背景: 売上入力特化のため顧客情報の入力手順が複雑という課題。予約時に顧客が自分で情報入力する形で解決する狙い
- Stripe連携（旧Phase 5）は予約システムの後に後回し

## インフラ方針（2026-07-17検討）
- 予約システムは既存アプリに組み込む（別アプリにしない）
- バックエンドはRailway継続（Render無料枠はスリープで予約用途に不向き、移行コストに見合わない）
- メール通知はResend無料枠（月3,000通・日100通）で当面十分

## 予約システム Step 1 実装・本番動作確認済み（2026-07-17）
- 公開予約ページ /book/[slug]・院側 /app/reservations・予約API一式（詳細はSPEC.md Phase 5）
- 本番E2E確認: カレンダー表示○・UI予約成立○・二重予約拒否○・枠反映○・管理一覧○・キャンセル○
- テスト予約3件は全てキャンセル済み（データはstatus=cancelledで残存）
- テスト院の予約URL: https://amatoseitai-v2.vercel.app/book/clinic_1_1782538545706

## 検討中（2026-07-17）
- ユーザーから「予約アプリにしたほうがいい、想像と違う」との反応 → どの部分が想像と違うのかヒアリング中
  （候補: 客側の見た目 / 院側管理画面を予約中心に / 完全別アプリ・別ブランド化）

## 次のフェーズ
- 予約システム Step 2以降（業務接続→メール通知→設定UI→SaaS基盤）
- 詳細は SPEC.md 参照
